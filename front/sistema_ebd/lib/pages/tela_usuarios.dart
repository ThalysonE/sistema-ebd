...
class _TelaUsuariosState extends ConsumerState<TelaUsuarios> {
  ...
  Future<void> fetchUsuariosComoMembros(int page) async {
    try{
      final resposta = await usuariosRequisicao.fetchUsuariosParaMembro(numeroPage: page, token: userLog.token, cargo: "");
  ...
  void initState() {
    ...
    fetchUsuariosComoMembros(numeroPage++).then((_){
      setState(() {
        isLoading = false;
      });
    });
    _controller.addListener(() {
      if (_controller.position.maxScrollExtent == _controller.offset) {
        if (usuarios.length < totalUsuarios) {
          ...
          fetchUsuariosComoMembros(numeroPage++);
        } else {
          setState(() {
            novosUsuarios = false;
          });
      ...
      child: ListTile(
        ...
        trailing:
            widget.temSelecao == false
                ...
                : Checkbox(
                  value: listaIdUsuariosSelecionados.contains(item.id),
                  onChanged: (value) {
                    if (value! == true) {
                      setState(() {
                      listaIdUsuariosSelecionados.add(item.id);
                      });
                    } else if (value == false) {
                      setState(() {
                        listaIdUsuariosSelecionados.remove(item.id);
                      });
                    }
                    print(listaIdUsuariosSelecionados);
                  },
                ),
      ),
    );
  ...
  void searchMembro(String query) async {
    ...
    _debounce = Timer(Duration(milliseconds: 700), () async {
      List<Membro>? resultado = await MembrosRepositories().searchMembro(nome: query, token: userLog.token);
      pesquisando = true;
      if (resultado == null) {
  ...
  Widget build(BuildContext context) {
    ...
    return Material(
      child: Scaffold(
        appBar: CriarAppBar(context, "Controle de Usuarios"),
        body: conteudo,
